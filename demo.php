...
include "nav1.php";
?>
<!-- <div class="total" style="margin-top:5rem;">
<div class="row mt-4">
<div class="col">
    <form action="" method="post" id="searchDiv">
        <div class="bbm">
            <input type="text" class="bm" id="search_chassis" placeholder="Search Name/Code" name="search" style="width:230px; height:35px; margin-left: 18pc; border-radius:10px; border:none;">
            <button class="sm" type="submit" name="submit1">Search</button>
    </div>
    </form>
    <script>
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Hide the search div by default
    searchDiv.style.display = "none";

    // Add an event listener to the "Types" dropdown
    typesDropdown.addEventListener("change", function() {
        // Check if "Sales" is selected
        if (typesDropdown.value === "Sales") {
            searchDiv.style.display = "block"; // Show the search div
        } else {
            searchDiv.style.display = "none"; // Hide the search div for other options
        }
    });
});
</script>
    ...
    <div class="container">
        <div class="table-container" style="margin-left: 4px; margin-bottom:-4%;">
        <table class="table"  id="myTable">
                <php
                if (isset($_POST['submit1'])) {
                    $search = $_POST['search'];

                    // Construct the SQL query to search for the customer
                    $customerQuery = "SELECT * FROM customer_master WHERE CUSTOMERCODE = '$search' OR CUSTOMERNAME LIKE '%$search%'";
                    ...
                    if (mysqli_num_rows($customerResult) > 0) {
                      ...
                      echo "<script>alert('Searched Customer Name is not found.');</script>";  
                    }
                }
                ?>
                </tbody>
...
const dataForm = document.querySelector(".form"); // Use querySelector to select the form
const inputs = dataForm.querySelectorAll("input[name], select[name]");
const tableContainer = document.querySelector(".table-container"); // Adjust this selector
const formContainer = document.querySelector(".container-box");
const popupTable = document.querySelector(".table");

// After clicking on a row in the table, populate the form fields
tableRows.forEach(row => {
row.addEventListener("click", function() {
const selectedId = this.getAttribute("data-id");
let rowData = {
customer_name: this.cells[1].textContent, // Get the value from the 2nd column
vehicle_name: this.cells[2].textContent, // VEHICLE NAME
customer_code: this.cells[0].textContent, // CUSTOMER CODE
phone_number: this.cells[3].textContent, // PHONE NUMBER
};
